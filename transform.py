...
def transform(aug, image, anns):
    ...
    :param image:   src 图
    :param anns:    标注信息
    :return:   增强后的img， 对应的ann
    '''
    image_shape = image.shape
    image = aug.augment_image(image)   #生成增强后的图片
    new_anns = []
    for ann in anns:
        ...
        new_ann = {'poly': poly, 'text': ann['text']}  #增强后的ann
...
def crop(image, anns, max_tries=10, min_crop_side_ratio=0.1):
    ...
    for ann in anns:
        points = np.round(ann['poly'], decimals=0).astype(np.int32)  # 四舍五入
