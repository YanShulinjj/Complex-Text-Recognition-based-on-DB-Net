...
def l1_loss(args, scale=10.):
    ...
    loss = K.switch(mask_sum > 0, tf.reduce_sum(tf.abs(pred - gt) * mask) / mask_sum, tf.constant(0.))  # 如果含
    loss = loss * scale
    return loss
...
def db_loss(args):
    ...
    l1_loss_ = l1_loss([thresh, thresh_map, thresh_mask])  # 概率图损失
    balanced_ce_loss_, dice_loss_weights = balanced_crossentropy_loss([binary, gt, mask]) # 二值图损失
